...
export default function DetailPanel({ 
  ...
  return (
    <div className="absolute inset-0 z-[1000] flex flex-col justify-end pointer-events-none">
      <div
        className={`bg-white rounded-t-2xl shadow-lift pointer-events-auto flex flex-col transition-all duration-300 ease-in-out ${
          isMinimized ? 'max-h-[64px] pb-2' : 'max-h-[70vh]'
        }`}
        style={{ borderTop: '1px solid #ECEAE4' }}
      >
        {/* دستگیره تاگل پنل (Minimize / Maximize) */}
        <div 
          className="flex justify-center items-center pt-2.5 pb-2 cursor-pointer shrink-0 hover:bg-slate-50 rounded-t-2xl transition-colors"
          onClick={() => setIsMinimized(!isMinimized)}
          title={isMinimized ? 'نمایش کامل منو' : 'کوچک کردن پنل برای مشاهده نقشه'}
        >
          <div className="w-12 h-1.5 rounded-full bg-slate-300" />
        </div>

        {/* حالت فشرده (Minimized) برای مشاهده مسیر و نقشه */}
        {isMinimized ? (
          <div className="px-5 flex items-center justify-between">
            ...
            >
              <span className="font-bold text-sm text-ink">{restaurant.name}</span>
              {totalCartCount > 0 && (
                <span className="text-xs bg-emerald-100 text-emerald-800 font-semibold px-2 py-0.5 rounded-full">
                  🛒 {totalCartCount} آیتم ({totalPayable.toLocaleString()} تومان)
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setIsMinimized(false)}
                className="text-xs font-semibold text-brand bg-brand-soft px-2.5 py-1 rounded-lg border-0 cursor-pointer hover:opacity-90"
              >
                باز کردن منو 🔼
              </button>
              <button
                onClick={onClose}
                className="w-7 h-7 rounded-full bg-surface border border-edge flex items-center justify-center text-ink-muted text-base hover:bg-edge cursor-pointer"
              >
                ×
              </button>
            </div>
          ...
          /* محتوای کامل منو، اطلاعات و دکمه ثبت سفارش */
          <div className="overflow-y-auto scrollbar-thin px-5 pb-6">
            <div className="flex gap-4 items-start mb-4">
              ...
              {/* کنترل‌های پنجره (دکمه پایین آوردن + بستن کامل) */}
              <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                <button
                  onClick={() => setIsMinimized(true)}
                  title="کوچک کردن برای مشاهده نقشه و مسیر"
                  className="w-7 h-7 rounded-full bg-surface border border-edge flex items-center justify-center text-ink-muted text-xs hover:bg-edge transition-colors cursor-pointer"
                >
                  🔽
                </button>
                <button
                  onClick={onClose}
                  title="بستن کامل"
                  className="w-7 h-7 rounded-full bg-surface border border-edge flex items-center justify-center text-ink-muted text-base hover:bg-edge transition-colors cursor-pointer"
                >
                  ×
                </button>
              </div>
